...
from time import sleep  # Used to space out print_lines and create Fake loading times
...
def check_xp(name, surname):
	...
	if person.XP + 1 > xp_needed:
		print_line(person.name, " has", person.XP, " XP")
		...
		if name not in used_names:
			...
			parent_2 = people[
				get_person_index(
					parent_2_first_name,
					parent_2_surname)]
			if parent_2.gender == "m": #Father's surname is used. So if second parent is father, makes him first parent.
...
def death(first_name,last_name):
	...
	index=get_person_index(
			...
			last_name)
	person=people[index]
	print(first_name,last_name," has died!")
	if isinstance(person,Player): #If player has died.
		end=1
	else:
		if person.assigned_room != "":  # Deals with if the person was assigned to any rooms.
			for r in rooms:
...
def rand_item(target_inventory):
	...
	if len(possible_items) > 0:
		...
		#print_line("Randomly adding",actual_item,"to",target_inventory,"inventory")
...
def add_to_inven(x, number, inven):
	...
	if x not in all_items:
		...
		# function is called.
		print_line("Item doesn't exist in the game's databases. Major bug with inventory adding system. Please contact dev")
	else:
		if inven == "player":
...
def scrap(it):
	...
	if it not in all_items:
		print_line("Bug with item scrapping system. Invalid argument passes to function. Please contact dev.")
	else:
